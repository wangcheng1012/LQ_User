...
import com.hyphenate.easeui.utils.GlideManagerUtils;
...
import static com.hyphenate.easeui.R.id.iv_userhead;
...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        if (message.direct() == Direct.SEND) {
            String headimg = message.getStringAttribute(EaseConstant.EXTRA_MY_HEAD_IMG, "");

            userAvatarView.setBackgroundColor(Color.TRANSPARENT);
            GlideManagerUtils.loadRoundImg(headimg, userAvatarView);
...
//                            itemClickListener.onUserAvatarClick(message.getFrom());
                            itemClickListener.onUserAvatarClick(message.getStringAttribute(EaseConstant.EXTRA_MY_UID, ""));
                        }
                    }
                }
            });
        ...
        activity.runOnUiThread(new Runnable() {
            public void run() {
                if (errorCode == EMError.MESSAGE_INCLUDE_ILLEGAL_CONTENT) {
                    Toast.makeText(activity, activity.getString(R.string.send_fail) + activity.getString(R.string
                            .error_send_invalid_content), Toast.LENGTH_SHORT).show();
                } else if (errorCode == EMError.GROUP_NOT_JOINED) {
                    Toast.makeText(activity, activity.getString(R.string.send_fail) + activity.getString(R.string
                            .error_send_not_in_the_group), Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(activity, activity.getString(R.string.send_fail) + activity.getString(R.string.connect_failuer_toast),
                            Toast.LENGTH_SHORT).show();
                }
